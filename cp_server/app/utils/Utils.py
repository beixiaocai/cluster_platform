...
def get_folder_size(folder_path):
    ...
    if os.path.isfile(folder_path):
        folder_size = round(os.path.getsize(folder_path),6)
    elif os.path.isdir(folder_path):
        for dir_path, sub_dirs, filenames in os.walk(folder_path):
            for filename in filenames:
                filepath = os.path.join(dir_path, filename)
                folder_size += round(os.path.getsize(filepath),6)

    folder_size_KB = folder_size / 1024
    folder_size_MB = folder_size_KB / 1024
    folder_size_GB = folder_size_MB / 1024
    # print("folder_path:",folder_path)
    # print("folder_size_KB:",folder_size_KB)
    # print("folder_size_MB:",folder_size_MB)
    # print("folder_size_GB:",folder_size_GB)

    return round(folder_size_GB,6)
